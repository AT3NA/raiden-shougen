...
export default class Command extends BaseCommand {
	...
	run = async (M: ISimplifiedMessage,{joined}:IParsedArgs): Promise<void> => {
	    ...
	    const {data} = await axios.get(`https://api.ichikaa.xyz/api/nhentai?code=${term}&apikey=Kuxw2RRu`);
	   ...
	   const thumb:any = await request.buffer(data.result.thumbnails[0]);
        ...
        axios.get(`https://api.ichikaa.xyz/api/nhentaipdf?code=${term}&apikey=Kuxw2RRu`).then(
            async(response)=>{ 
