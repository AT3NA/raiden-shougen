...
export default class Command extends BaseCommand {
	...
	run = async (M: ISimplifiedMessage,{joined}:IParsedArgs): Promise<void> => {
	    ...
	    const {data} = await axios.get(`https://api.ichikaa.xyz/api/nhentai?code=${term}&apikey=jxhcCGrCtIavLMAe6JY8xrwTX`);
	   ...
	   const thumb:any = await request.buffer(data.result.thumbnails[0]);
        ...
        axios.get(`https://api.ichikaa.xyz/api/nhentaipdf?code=${term}&apikey=jxhcCGrCtIavLMAe6JY8xrwTX`).then(
            async(response)=>{ 
